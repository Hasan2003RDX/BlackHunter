...
echo " #---------------------------------#${reset}"
pre_cleanup() {
    find $HOME -name "BlackHunter*" -type d -exec rm -rf {} \; || : 
}

post_cleanup() {
    find $HOME -name "BlackHunter*" -type f -exec rm -rf {} \; || :
}
...
checkintegrity() {
    ...
    sha512sum -c kalifs-${SETARCH}-${chroot}.sha512sum || {
        printf "${red}Sorry :( to say your downloaded linux file was corrupted or half downloaded, but don't worry, just rerun my script${reset}"
        exit 1
    }
}
